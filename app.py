        ...
        with st.expander("💡 了解 AI 白盒解释器如何计算风险？(以沛县某店铺示例)", expanded=True):
            ...
            with col_ex2:
                ...
                * **特征碰撞 (35.4%)**：算法提取经营范围文本，发现高关联词簇。
                    * `百货 (20.2%)`：该词在目标文本中 TF-IDF 词频权重达到 0.85，且模型统计发现历史违规户中含有“百货”的基尼不纯度（分类区分度）达到 0.48（满分 0.5），两者相乘并经过公式计算最终贡献了 20.2% 的嫌疑率。
                    * `批发 (15.2%)`：基于语义空间的上下文共现频率（词频权重 0.72），该词触发惩罚阈值，公式计算后贡献 15.2%。
                * **语义关联 (28.4%)**：店名分词触发警报。
                    * `超市 (28.4%)`：AI 检测到该店铺名称属于典型的“隐蔽售烟业态”（词频 0.91），在 100 棵决策树的联合投票中，有 73 棵树因为该特征将其推向“高风险叶子节点”，公式计算概率占比 28.4%。
                * **信用偏离 (24.7%)**：非文本维度数值惩罚。
                    * `信用偏离 (24.7%)`：因该商户信用评分仅为 42 分，经过 MinMaxScaler 归一化后得分为 0.12（分布在 0-1 区间），极大偏离了正规店铺 0.75-0.90 的特征分布中位数，模型依据公式计算给予 24.7% 的危险权重。
                * **闭环判定**：综合上述多维特征权重（35.4% + 28.4% + 24.7% = 88.5%），模型最终锁定该商户。
                """)
                
            st.markdown("---")
            # 注意这里使用了 r"""...""" 以支持 LaTeX 公式在 Streamlit 中无缝渲染
            st.markdown(r"""
            #### 📚 核心专业名词解释与底层计算公式
            ...
            * **基尼不纯度（分类区分度）**：衡量特征分类“干净程度”的指标。数值越低说明这个词能越明确地区分“无证户”和“合规户”。
              * **公式核心**：$$Gini = 1 - \sum_{i=1}^{C} p_i^2$$ （$p_i$ 为该节点中各类别的概率）
            ...
            * **特征分布**：指某项数据在整个商户群体中的聚集规律区间。一旦某商户的数值偏离了正常聚集的中心点，就会被算法迅速捕捉为异常离群点。
              * **偏离度参考（Z-score）**：$$Z = \frac{X - \mu}{\sigma}$$ （$\mu$ 为均值，$\sigma$ 为标准差）
            """)
