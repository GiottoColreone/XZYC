...
def draw_analysis_charts(df, t_font, l_font):
    ...
    # 增加 figsize 确保空间充足，减少重叠
    fig1, axes1 = plt.subplots(2, 3, figsize=(15, 8.5))
    ...
    axes1[1, 2].pie([high_risk_reps, df.shape[0] - high_risk_reps], labels=None, autopct='%1.1f%%', colors=['#FF6B6B', '#4ECDC4'], startangle=140, textprops={'fontproperties': l_font})
    axes1[1, 2].set_title('法人身份识别比例', fontproperties=t_font)
    axes1[1, 2].legend(['高危法人', '普通法人'], prop=l_font, loc="center left", bbox_to_anchor=(1.0, 0.5))
...
if start_btn:
    if not file_biz or not file_unl:
        ...
        log_to_terminal("[NLP] 正在对 [公司名称] 执行高维空间映射...")
        X_name = vec_name.fit_transform(df_all['公司名称'])
        log_to_terminal("[NLP] 正在对 [经营范围] 执行高维空间映射...")
        ...
        for i in range(weighted_X.shape[0]):
            ...
            if row.nnz > 0:
                ...
                explanations.append(" + ".join(expl_parts) if expl_parts else "无显著异常")
            else:
        ...
        log_to_terminal("[SYSTEM] ✅ 演算闭环结束！正在生成终端高危打击清单与数据大屏...")

        # --- 结果展示区 ---
        st.success("🎯 稽查演算已收官！")
        c1, c2, c3 = st.columns(3)
        c1.metric("极高风险锁定", f"{len(target_pool[target_pool['风险等级']=='极高风险'])} 家", "重点打击")
        ...
        st.download_button("📥 导出全量排查建议名单 (Excel)", buffer, "AI稽查名单.xlsx", "application/vnd.ms-excel")
